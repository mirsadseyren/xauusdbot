...
class POIGenerator:
    ...
    def generate_pois(self, df_4h: pd.DataFrame) -> List[POI]:
        ...
        # We need integer indexing to look back/forward easily
        times = df_4h.index
        ...
        while i < len(df_4h):
            ...
            prev_is_doji = closes[i-1] == opens[i-1]
            
            # SHORT POI (Supply) in Downtrend
            if is_downtrend:
                if is_green and (prev_is_red or prev_is_doji):
            ...
            # LONG POI (Demand) in Uptrend
            elif is_uptrend:
                if is_red and (prev_is_green or prev_is_doji):
                    lookback_start = max(0, i - self.max_lookback)
                    highest_high = highs[i]
                    for j in range(i, lookback_start - 1, -1):
